...
class TestNotifyTransitionTrackerUnit:
    def test_first_evaluation_worthy_is_a_transition(self) -> None:
        ...
        assert t.transitioned("203.0.113.10", band_met=True, tier=None, tier_axis_enabled=False) is True

    def test_first_evaluation_not_worthy_is_not_a_transition(self) -> None:
        t = NotifyTransitionTracker()
        assert t.transitioned("203.0.113.10", band_met=False, tier=None, tier_axis_enabled=False) is False
    ...
    def test_tier_enters_from_no_tier_fires(self) -> None:
        t = NotifyTransitionTracker()
        assert t.transitioned("203.0.113.10", band_met=False, tier=2, tier_axis_enabled=True) is True
    ...
    def test_independent_actors_tracked_separately(self) -> None:
        ...
        assert t.transitioned("203.0.113.11", band_met=True, tier=None, tier_axis_enabled=False) is True

    def test_reset_single_actor_clears_only_that_actor(self) -> None:
        t = NotifyTransitionTracker()
        t.transitioned("203.0.113.10", band_met=True, tier=None, tier_axis_enabled=False)
        t.transitioned("203.0.113.11", band_met=True, tier=None, tier_axis_enabled=False)
        t.reset("203.0.113.10")
        # 203.0.113.10 looks brand-new again -> transitions; 203.0.113.11 stays remembered.
        assert t.transitioned("203.0.113.10", band_met=True, tier=None, tier_axis_enabled=False) is True
        assert t.transitioned("203.0.113.11", band_met=True, tier=None, tier_axis_enabled=False) is False
